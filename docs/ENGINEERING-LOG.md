...
## Losing a seat race showed no message at all

**Symptom.** Two customers select the same seat; one loses it to the other before clicking Hold. The losing customer's selection silently disappeared on the next poll — no error, no "taken by another customer," nothing. Confirmed with a forced-loss reproduction (one session holds a seat, a second session selects a different seat, a third session steals that exact seat via a direct API call, then the routine 3-second poll runs) — polling itself was verified working (8 requests in a 10-second window) before the real cause was found, ruling out "polling silently stopped."

**Root cause.** `ShowSeatMapClient.tsx`'s seat-loss detection effect built the list of newly-lost seats (`lostNow`) by pushing into it from *inside* the callback passed to `setSelectedSeatIds(prev => ...)`, then read `lostNow.length` on the very next line, outside that callback. React does not guarantee a functional state updater runs synchronously before the next statement executes. The seat still got deselected correctly (that part of the updater did run, eventually) but `lostNow` was read before it had been populated, so the notice condition never fired — every time, not intermittently, which is why forcing the scenario reproduced it reliably.

**Fix.** Added a ref (`selectedSeatIdsRef`) kept in sync with the `selectedSeatIds` state on every render, and rewrote the detection as a plain synchronous loop over that ref's current value, calling `setSelectedSeatIds` with the already-computed result instead of a functional updater. Verified against a local production build and against the live deployment with both a pure-selection-stolen-via-poll scenario and a Hold-button-click-time conflict — both now show plain-language copy ("Seat A2 was just taken by another customer" / "Some requested seats were just taken by another customer"), never a raw status code.
